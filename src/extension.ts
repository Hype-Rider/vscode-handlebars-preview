...
export function activate(context: ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    commands.registerCommand("handlebarsPreview.preview", () => {
      const fileExtensionsToPreview = config.get<string[] | undefined>(
      ...
      workspace.onDidSaveTextDocument((e) => {
        ...
        outputChannel.appendLine(
          "name of file that was just saved:" + fileName
        );
        outputChannel.appendLine(
          "extensions to trigger preview:" + fileExtensionsToPreview?.join(", ")
        );
...
export interface HelperFunctionMap {
  [key: string]: Handlebars.HelperDelegate;
}
export interface Context {
  data: object;
  helperFns: HelperFunctionMap;
}
...
function makeWebviewBody(activeDocument: TextDocument): string {
  ...
  try {
    ...
    outputChannel.appendLine(
      JSON.stringify(
        Object.entries<Handlebars.HelperDelegate>(context.helperFns).map(
          ([key, value]) => ({
            key,
            body: value.toString(),
          })
...
export function renderTemplate(template: string, context: Context) {
  ...
  Object.entries<Handlebars.HelperDelegate>(context.helperFns).forEach(
    ([key, value]) => {
...
function specifiedContext(doc: TextDocument): ContextResolutionResult {
  ...
  const matches =
    /\{\{!--\s*vscode-handlebars-preview-context-source=(.*)\s*--\}\}/.exec(
      docFirstLine
    );
  ...
  try {
    ...
    if (!ctx.data)
      return {
        error: `Expected field data in context: ${JSON.stringify(
          ctx,
          null,
          2
        )}`,
    ...
    if (!ctx.helperFns)
      return {
        error: `Expected field helperFns in context: ${JSON.stringify(
          ctx,
          null,
          2
        )}`,
